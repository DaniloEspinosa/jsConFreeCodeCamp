/*
Tenemos un objeto que representa parde de una coleeción de álbumes musicales.

Cada álbum tiene un número de identificación unico (id) asociado a otras propiedades.

No todos los álbumes tienen la informacion completa.
*/
...
-discos (el objeto representa la colección de discos).
-id.
...
-Si "propiedad" es igual a la cadena de caracteres "canciones" pero el álbum no tiene una propiedad 
llamada "canciones", crea un arreglo vacío y agrega "valor" a ese arreglo.

-Si "propiedad" es igual a la cadena de caracteres "canciones" y "valor" no es una cadena vacía, agrega "valor" al final 
del arreglo de canciones del álbum correspondiente.

-Si "valor" no es una cadena vacía y "propiedad" no es igual a "canciones", asigna el valor del párametro "valor" a la propiedad.
Si la propiedad no existe, debes crearla y asignar este valor.
*/

/*
// Esta es mi versión de la solución
function actualizarDiscos(discos, id, propiedad, valor) {
  if (valor === "") {
    delete discos[id][propiedad];
  } else if (propiedad === "canciones" && !discos[id]["canciones"]) {
    discos[id][propiedad] = [];
...
// Esta es la versión de freeCodeCamp
